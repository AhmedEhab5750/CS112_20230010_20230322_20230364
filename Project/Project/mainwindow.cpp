...
MainWindow::MainWindow(QWidget *parent) : QWidget(parent) {
    ...
    // Create the container widget for the image and buttons
    QWidget *containerWidget = new QWidget(this);
    QHBoxLayout *containerLayout = new QHBoxLayout(containerWidget);

    // Create the image label
    imageLabel = new QLabel(this);
    QPixmap image("path_to_your_image.jpg"); // Replace "path_to_your_image.jpg" with the path to your image file
    ...
    // Add load image buttons
    addButton("Load Image 1", [this]() { load_image(1); });
    addButton("Load Image 2", [this]() { load_image(2); });
    addButton("Save Image", [this]() { save_image(); });
    addButton("Grey Filter", [this]() { to_grey_f1(image1); });
    addButton("Black & White Filter", [this]() { to_black_and_white_f2(image1); });
    addButton("Invert Colors Filter", [this]() { invert_colors_f3(image1); });
    addButton("Flip Horizontally", [this]() { flip_horizontally_f5(image1); });
    addButton("Flip Vertically", [this]() { flip_vertically_f5(image1); });
    addButton("Blur Image", [this]() { boxBlur_f12(image1, 20); });
    addButton("Brightness 50% more", [this]() { brightness_f7(image1, 1); });
    addButton("Brightness 50% less", [this]() { brightness_f7(image1, 1); });
    addButton("Brightness 50% less", [this]() { brightness_f7(image1, 3, 100); });
    addButton("Frame Red Adder", [this]() { addFrame_f9(image1, 'A'); });
    addButton("Frame Green Adder", [this]() { addFrame_f9(image1, 'B'); });
    addButton("Frame Blue Adder", [this]() { addFrame_f9(image1, 'C'); });
    addButton("White and Blue Frame Adder", [this]() { W_and_B_Frame_f9(image1, 10, 20); });
    addButton("Merge Images choice 1", [this]() { mergeImages_f4(image1,image2, 1); });
    addButton("Merge Images choice 2", [this]() { mergeImages_f4(image1,image2, 2); });
    addButton("Crop Image", [this]() { crop_f8(image1, 10, 10, 100, 100); });
    addButton("Rotate Image", [this]() { rotate_left_f6(image1); });
    addButton("Rotate Image", [this]() { rotate_right_f6(image1); });
    addButton("Resize Image", [this]() { resize_image_f11(image1, 2000, 2000); });
    addButton("Detect Edges", [this]() { detectEdges_f10(image1); });
    addButton("Sun Light Filter", [this]() { wano_Sunlight_f13(image1); });
    addButton("Purple Night Filter", [this]() { wano_purple_f16(image1); });
    addButton("TV Noise Filter", [this]() { applyTVNoiseFilter_f15(image1); });
    addButton("Oil Painting Filter", [this]() { oilPaintingFilter_faster_f14(image1, 10, 100); });
    addButton("Infra Red Filter", [this]() { to_InfraRed_f17(image1); });
    addButton("WhiteRed Filter", [this]() { to_whiteRed_filter_extra1(image1); });
    addButton("Blood Filter", [this]() { to_Blood_filter_extra2(image1); });
    addButton("WhiteRed Filter", [this]() { to_redGreen_extra3(image1); });
    addButton("Xray Filter", [this]() { to_Xray_filter_extra4(image1); });
    addButton("Skew Image Filter", [this]() { skewImagealmostright_f18(image1, 45); });
...
void MainWindow::addButton(const QString &text, std::function<void()> onClick) {
    QPushButton *button = new QPushButton(text, this);
    connect(button, &QPushButton::clicked, onClick);
    buttonsLayout->addWidget(button);
}
void MainWindow::to_grey_f1(Image &image){
...
void MainWindow::to_black_and_white_f2(Image &image1){
...
void MainWindow::invert_colors_f3(Image &image1){
    for (int i = 0; i < image1.width; i++) {
        for (int j = 0; j < image1.height; j++) {
            for (int k = 0; k < image1.channels; k++) {
                image1(i, j, k) *= -1;
...
void MainWindow::mergeImages_f4(Image& image1, Image& image2, int choice) {
    int new_width;
    int new_height;
    if(choice == 1){
...
void MainWindow::flip_horizontally_f5(Image &image1){
...
void MainWindow::flip_vertically_f5(Image &image1){
...
void MainWindow::rotate_right_f6(Image &image1){
    Image new_image(image1.height, image1.width); // Flipping width and height
    for (int i = 0; i < image1.width; i++) {
        for (int j = 0, rj = image1.height - 1; j < image1.height; j++, rj--) {
            for (int k = 0; k < image1.channels; k++) {
                new_image(rj, image1.width - i - 1, k) = image1(i, j, k); // Adjusted indexing for temp
            }
        }
    }
    new_image.saveImage("temp.jpg");
...
void MainWindow::rotate_left_f6(Image &image1){
    Image new_image(image1.height, image1.width); // Flipping width and height
    for (int i = 0; i < image1.width; i++) {
        for (int j = 0; j < image1.height; j++) {
            for (int k = 0; k < image1.channels; k++) {
...
void MainWindow::brightness_f7(Image &image, int choice, float number) {
    if (choice == 1) {
        for (int i = 0; i < image.width; i++) {
            for (int j = 0; j < image.height; j++) {
                for (int k = 0; k < image.channels; k++) {
...
void MainWindow::crop_f8(Image &image, int x, int y, int W, int H){
    Image cropped(W,H);
...
void MainWindow::addFrame_f9(Image& image, char choice) {
...
void MainWindow::W_and_B_Frame_f9(Image& image, int frameWidth, int checkerSize) {
    for (int y = 0; y < image.height; ++y) {
        for (int x = 0; x < image.width; ++x) {
...
void MainWindow::detectEdges_f10(Image& image) {
...
void MainWindow::resize_image_f11(Image &image, int new_width,int new_height) {
...
void MainWindow::boxBlur_f12(Image& image,int blurRadius) {
...
void MainWindow::wano_Sunlight_f13(Image& img){
...
void MainWindow::applyOilPaintingFilter_f14(Image& image) {
...
void MainWindow::oilPaintingFilter_faster_f14(Image& image, int radius, int intensity) {
...
void MainWindow::applyTVNoiseFilter_f15(Image& image) {
...
void MainWindow::wano_purple_f16(Image& img){
    for (int i = 0; i < img.width; ++i) {
        for (int j = 0; j < img.height; ++j) {
            img(i, j, 1) *= 0.7;
        }
...
void MainWindow::to_InfraRed_f17(Image &image){
    for (int i = 0; i < image.width; i++) {
        for (int j = 0; j < image.height; j++) {
            int red = image(i, j, 0);
...
void MainWindow::skewImagealmostright_f18(Image& inputImage, double skewDegree) {
...
void MainWindow::to_whiteRed_filter_extra1(Image &image){ // BETA
    for (int i = 0; i < image.width; i++) {
        for (int j = 0; j < image.height; j++) {
            int red = image(i, j, 0);
            if(red >= 250){
...
void MainWindow::to_Blood_filter_extra2(Image &image){ // BETA
    for (int i = 0; i < image.width; i++) {
        for (int j = 0; j < image.height; j++) {
...
void MainWindow::to_redGreen_extra3(Image &image){ // BETA still not working as intended.
    for (int i = 0; i < image.width; i++) {
        for (int j = 0; j < image.height; j++) {
            int intensity = (image(i, j, 0) + image(i, j, 1) + image(i, j, 2)) / 3;
            int red = intensity;  // Red component
...
void MainWindow::to_Xray_filter_extra4(Image &image){ // BETA
    for (int i = 0; i < image.width; i++) {
        for (int j = 0; j < image.height; j++) {
            int red = image(i, j, 0);
...
void MainWindow::save_image() {
    QString saveName = QInputDialog::getText(this, "Save Image", "Enter the name of the edited image\nOnly Supported Extensions are {.jpg, .bmp, .png, .tga}\nTo save:");
    if (!saveName.isEmpty()) {
        try {
            image1.saveImage(saveName.toStdString());
        } catch(...) {
            // Handle exceptions here
        }
    }
}
void MainWindow::load_image(int imageNumber) {
    QString imagePath = QInputDialog::getText(this, "Load Image", "Enter the path to the image:");
    if (!imagePath.isEmpty()) {
        try {
            if (imageNumber == 1) {
                ...
                image2.loadNewImage(imagePath.toStdString());
            }
            // Handle other image slots if needed
        } catch(...) {
            // Handle exceptions here
        }
    }
